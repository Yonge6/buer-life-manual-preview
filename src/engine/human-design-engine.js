import { SwissEphemeris } from "../../vendor/swisseph/swisseph-browser.js?v=23ad4682e3f92469";
import { getIncarnationCross } from "../../vendor/natalengine/incarnation-crosses.js?v=23ad4682e3f92469";
...
async function getSwissEphemeris() {
  if (!swePromise) {
    swePromise = (async () => {
      ...
      await swe.init(new URL("../../vendor/swisseph/swisseph.wasm?v=23ad4682e3f92469", import.meta.url).href);
      await swe.loadEphemerisFiles(EPHEMERIS_FILES.map((name) => ({
        name,
        url: new URL(`../../vendor/swisseph/ephe/${name}?v=23ad4682e3f92469`, import.meta.url).href,
      })));
      return swe;
    })();
  }
