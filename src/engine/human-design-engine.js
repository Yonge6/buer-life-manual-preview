import { SwissEphemeris } from "../../vendor/swisseph/swisseph-browser.js?v=c79c223bad4d5a00";
import { getIncarnationCross } from "../../vendor/natalengine/incarnation-crosses.js?v=c79c223bad4d5a00";
...
async function getSwissEphemeris() {
  if (!swePromise) {
    swePromise = (async () => {
      ...
      await swe.init(new URL("../../vendor/swisseph/swisseph.wasm?v=c79c223bad4d5a00", import.meta.url).href);
      await swe.loadEphemerisFiles(EPHEMERIS_FILES.map((name) => ({
        name,
        url: new URL(`../../vendor/swisseph/ephe/${name}?v=c79c223bad4d5a00`, import.meta.url).href,
      })));
      return swe;
    })();
  }
