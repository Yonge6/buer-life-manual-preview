import { SwissEphemeris } from "../../vendor/swisseph/swisseph-browser.js?v=5a88248da045c30c";
import { getIncarnationCross } from "../../vendor/natalengine/incarnation-crosses.js?v=5a88248da045c30c";
...
async function getSwissEphemeris() {
  if (!swePromise) {
    swePromise = (async () => {
      ...
      await swe.init(new URL("../../vendor/swisseph/swisseph.wasm?v=5a88248da045c30c", import.meta.url).href);
      await swe.loadEphemerisFiles(EPHEMERIS_FILES.map((name) => ({
        name,
        url: new URL(`../../vendor/swisseph/ephe/${name}?v=5a88248da045c30c`, import.meta.url).href,
      })));
      return swe;
    })();
  }
