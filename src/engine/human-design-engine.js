import { SwissEphemeris } from "../../vendor/swisseph/swisseph-browser.js?v=c6b782a4d10072ed";
import { getIncarnationCross } from "../../vendor/natalengine/incarnation-crosses.js?v=c6b782a4d10072ed";
...
async function getSwissEphemeris() {
  if (!swePromise) {
    swePromise = (async () => {
      ...
      await swe.init(new URL("../../vendor/swisseph/swisseph.wasm?v=c6b782a4d10072ed", import.meta.url).href);
      await swe.loadEphemerisFiles(EPHEMERIS_FILES.map((name) => ({
        name,
        url: new URL(`../../vendor/swisseph/ephe/${name}?v=c6b782a4d10072ed`, import.meta.url).href,
      })));
      return swe;
    })();
  }
