import { SwissEphemeris } from "../../vendor/swisseph/swisseph-browser.js?v=ff80ed6a5cb2c519";
import { getIncarnationCross } from "../../vendor/natalengine/incarnation-crosses.js?v=ff80ed6a5cb2c519";
...
async function getSwissEphemeris() {
  if (!swePromise) {
    swePromise = (async () => {
      ...
      await swe.init(new URL("../../vendor/swisseph/swisseph.wasm?v=ff80ed6a5cb2c519", import.meta.url).href);
      await swe.loadEphemerisFiles(EPHEMERIS_FILES.map((name) => ({
        name,
        url: new URL(`../../vendor/swisseph/ephe/${name}?v=ff80ed6a5cb2c519`, import.meta.url).href,
      })));
      return swe;
    })();
  }
