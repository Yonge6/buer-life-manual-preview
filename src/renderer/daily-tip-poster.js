import { formatDailyTipText } from "../app/daily-tip.js?v=c79c223bad4d5a00";
...
export async function createDailyTipPoster({ tip, language, date = new Date() }) {
  ...
  const [qr, hero, orb] = await Promise.all([
    ...
    loadImage(new URL('../../assets/buer-aurora-hero.webp?v=c79c223bad4d5a00', import.meta.url).href).catch(() => null),
    loadImage(new URL("../../assets/buer-orb-v2.png?v=c79c223bad4d5a00", import.meta.url).href),
